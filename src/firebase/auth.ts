...
function decodeJwtPayload(token: string): any | null {
  try {
    ...
    const json = typeof window !== "undefined" && typeof atob === "function"
      ? atob(normalized)
      ...
      try {
        ...
        try {
          ...
          if (likelyMismatch) {
            ...
            throw new Error(
              `CREDENTIAL_MISMATCH: Custom token was minted for project "${tokenProjectId ?? "<unknown>"}" but you are signing into "${appProjectId}". ` +
