...
// src/inventory/reports/index.ts
var reports_exports = {};
__export(reports_exports, {
  GetReportsCustomerOrdersGET: () => GET,
  GetReportsOrderSalesGET: () => GET2,
  getReportsCustomerOrders: () => getReportsCustomerOrders,
  getReportsOrderSales: () => getReportsOrderSales
});
...
async function getReportsCustomerOrders(query) {
  if (typeof window === "undefined") {
    ...
    const { Api: Api2 } = await Promise.resolve().then(() => (init_api(), api_exports));
    return getWithAuth2(Api2.getReportsCustomerOrders, query);
  }
  const params = new URLSearchParams();
  if (query) {
...
async function getReportsOrderSales(query) {
  if (typeof window === "undefined") {
    const { getWithAuth: getWithAuth2 } = await Promise.resolve().then(() => (init_fetcher(), fetcher_exports));
    const { Api: Api2 } = await Promise.resolve().then(() => (init_api(), api_exports));
    return getWithAuth2(Api2.getReportsOrderSales, query);
  }
  const params = new URLSearchParams();
  if (query) {
    Object.entries(query).forEach(([k, v]) => {
...
0 && (module.exports = {
  GetReportsCustomerOrdersGET,
  GetReportsOrderSalesGET,
