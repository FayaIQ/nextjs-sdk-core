...
var getMenusDropdown_exports = {};
__export(getMenusDropdown_exports, {
  GET: () => GET
});
module.exports = __toCommonJS(getMenusDropdown_exports);
...
async function getMenusDropdown() {
  if (typeof window === "undefined") {
    ...
    const { Api: Api2 } = await Promise.resolve().then(() => (init_api(), api_exports));
    return getWithAuth2(`${Api2.getMenusDropdown}`, {});
  } else {
...
async function GET(request) {
  try {
    ...
    return import_server.NextResponse.json(menus);
  } catch (error) {
