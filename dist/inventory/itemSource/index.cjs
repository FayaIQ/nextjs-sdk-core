...
// src/inventory/itemSource/index.ts
var itemSource_exports = {};
__export(itemSource_exports, {
  ItemSourcesGET: () => GET,
  getItemsSource: () => getItemsSource
});
module.exports = __toCommonJS(itemSource_exports);
...
async function getItemsSource() {
  if (typeof window === "undefined") {
    ...
    const { Api: Api2 } = await Promise.resolve().then(() => (init_api(), api_exports));
    return getWithAuth2(`${Api2.getItemsSource}`, {});
  } else {
...
async function GET(request) {
  try {
    ...
    return import_server.NextResponse.json(itemsSource);
  } catch (error) {
