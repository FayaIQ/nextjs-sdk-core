...
var postCopyParentStore_exports = {};
__export(postCopyParentStore_exports, {
  POST: () => POST
});
module.exports = __toCommonJS(postCopyParentStore_exports);
...
async function POST(request) {
  try {
    ...
    if (!itemIds) {
      return import_server2.NextResponse.json({ error: "itemIds array is required" }, { status: 400 });
    }
