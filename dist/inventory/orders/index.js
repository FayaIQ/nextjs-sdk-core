import {
  ...
  Sign,
  getOrders
} from "../../chunk-IB3CZ3SW.js";
import {
  Api
} from "../../chunk-QSBWKE5H.js";
import {
  postWithAuth,
  putWithAuth
} from "../../chunk-GBLWFIA2.js";
...
async function getOrder(id) {
  if (typeof window === "undefined") {
    ...
    const { Api: Api2 } = await import("../../api-RHFGMX5X.js");
    return getWithAuth(
      `${Api2.getOrder(id)}`
    );
  }
...
async function postOrder(orderData, options) {
  if (typeof window === "undefined") {
    try {
      ...
      if (error.status === 403) {
        console.error("\u{1F512} Authentication failed. Please check:");
...
async function putOrderApprove(id, note) {
  if (typeof window === "undefined") {
    const { putWithAuth: putWithAuth2 } = await import("../../fetcher-OYBMXYXN.js");
    const { Api: Api2 } = await import("../../api-RHFGMX5X.js");
    return putWithAuth2(
...
async function getOrdersFullInfo(input) {
  ...
  if (typeof window === "undefined") {
    const { postWithAuth: postWithAuth2 } = await import("../../fetcher-OYBMXYXN.js");
    const { Api: Api2 } = await import("../../api-RHFGMX5X.js");
...
async function POST(request) {
  try {
    ...
    console.log("\u{1F4E6} Received order data:", JSON.stringify(orderData, null, 2));
    if (!orderData.address || !orderData.orderItems || !Array.isArray(orderData.orderItems)) {
    ...
    console.log("\u{1F511} Using headers:", Object.keys(headers));
    ...
    if (error && typeof error === "object") {
      ...
      if (apiError.status) {
        ...
        if (apiError.status === 403) {
          errorMessage = "Authentication failed. Please ensure you're logged in and have permission to create orders.";
          console.error("\u{1F512} 403 Forbidden - Token might be invalid, expired, or missing required permissions");
      ...
      if (apiError.body) {
        ...
        console.error(
          "\u{1F4C4} API Error Body:",
          JSON.stringify(apiError.body, null, 2)
        );
      }
      if (apiError.message && apiError.status !== 403 && apiError.status !== 401) {
    ...
    console.error("\u{1F4A5} Final error message:", errorMessage);
    return NextResponse.json(
      {
        error: errorMessage,
