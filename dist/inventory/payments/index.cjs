...
// src/inventory/payments/index.ts
var payments_exports = {};
__export(payments_exports, {
  DeletePaymentDELETE: () => DELETE,
  GetPaymentByIdGET: () => GET3,
  GetPaymentsGET: () => GET,
  GetPaymentsReportGET: () => GET2,
  GetStorePaymentsGET: () => GET4,
  PostPaymentPOST: () => POST,
  PutPaymentPUT: () => PUT,
  deletePayment: () => deletePayment,
  getPaymentById: () => getPaymentById,
  getPayments: () => getPayments,
  getPaymentsReport: () => getPaymentsReport,
  getStorePayments: () => getStorePayments,
...
async function getPayments(query) {
  if (typeof window === "undefined") {
    ...
    const { Api: Api2 } = await Promise.resolve().then(() => (init_api(), api_exports));
    return getWithAuth2(Api2.getPayments, query);
  }
  const params = new URLSearchParams();
...
async function getPaymentById(id) {
  if (typeof window === "undefined") {
    const { getWithAuth: getWithAuth2 } = await Promise.resolve().then(() => (init_fetcher(), fetcher_exports));
    const { Api: Api2 } = await Promise.resolve().then(() => (init_api(), api_exports));
    return getWithAuth2(Api2.getPayment(id));
  }
...
async function postPayment(payload) {
  if (typeof window === "undefined") {
    ...
    const { Api: Api2 } = await Promise.resolve().then(() => (init_api(), api_exports));
    return postWithAuth2(Api2.postPayments, payload);
  }
...
async function putPayment(id, payload) {
  if (typeof window === "undefined") {
    ...
    const { Api: Api2 } = await Promise.resolve().then(() => (init_api(), api_exports));
    return putWithAuth2(Api2.putPayment(id), payload);
  }
  const res = await fetch(`/api/payments/${id}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
...
async function deletePayment(id) {
  if (typeof window === "undefined") {
    ...
    const { Api: Api2 } = await Promise.resolve().then(() => (init_api(), api_exports));
    return deleteWithAuth2(Api2.deletePayment(id));
  }
...
async function getStorePayments(storeId) {
  if (typeof window === "undefined") {
    const { getWithAuth: getWithAuth2 } = await Promise.resolve().then(() => (init_fetcher(), fetcher_exports));
    const { Api: Api2 } = await Promise.resolve().then(() => (init_api(), api_exports));
    return getWithAuth2(Api2.getStorePayments(storeId));
  }
...
async function getPaymentsReport(query) {
  if (typeof window === "undefined") {
    const { getWithAuth: getWithAuth2 } = await Promise.resolve().then(() => (init_fetcher(), fetcher_exports));
    const { Api: Api2 } = await Promise.resolve().then(() => (init_api(), api_exports));
    return getWithAuth2(Api2.getPaymentsReport, query);
  }
  const params = new URLSearchParams();
  if (query) Object.entries(query).forEach(([k, v]) => {
    if (v !== void 0 && v !== null) params.append(k, String(v));
