...
async function getTokenImpl() {
  ...
  if (USE_TOKEN_ROUTE && typeof window === "undefined") {
    try {
      console.log("[token:getToken] no cookie found, calling /api/auth/token ONCE to set cookie");
  ...
  try {
    if (typeof window !== "undefined") {
      const { getFirebaseIdToken } = await import("./auth-AFSBYWJH.js");
      const freshToken = await getFirebaseIdToken(true);
      if (freshToken) {
        console.log("[token:getToken] refreshed Firebase token from current user");
        thirdPartyToken = freshToken;
      }
    } else {
      const { cookies } = await import("next/headers");
      const cookie = await cookies();
  ...
  try {
    if (typeof window === "undefined") {
      ...
      if (AUTH_MODE === "auto") {
        try {
          cookieStore.set("isUser", "0", { path: "/", maxAge: 60 * 60 });
        } catch {
        }
      }
      console.log("[token:getToken] access_token persisted in server cookies");
    }
  } catch (e) {
    console.log("[token:getToken] failed to persist access_token cookie", e);
  }
