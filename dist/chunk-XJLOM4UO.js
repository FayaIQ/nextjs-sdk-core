...
async function startPhoneSignIn(phoneNumber, options) {
  ...
  const { getSecondaryApp } = await import("./config-QT4SQ6SK.js");
  ...
  return {
    confirm: async (code) => {
      ...
      try {
        ...
        const { getPrimaryApp } = await import("./config-QT4SQ6SK.js");
        ...
        try {
          ...
          const tokenAud = payload.aud || payload.project_id;
          const code2 = e?.code || e?.message || String(e);
          const likelyMismatch = code2?.includes("custom-token-mismatch") || code2?.includes("invalid-custom-token") || code2?.includes("CREDENTIAL_MISMATCH");
          if (likelyMismatch) {
            ...
            throw new Error(
              `CREDENTIAL_MISMATCH: Custom token was minted for project "${tokenAud}" but you are signing into "${appProjectId}". Ensure your verifySMS Cloud Function mints tokens using the PRIMARY project's service account (the same project used by getPrimaryApp).`
            );
          }
          throw e;
        }
