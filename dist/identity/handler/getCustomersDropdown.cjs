...
var getCustomersDropdown_exports = {};
__export(getCustomersDropdown_exports, {
  GET: () => GET
});
module.exports = __toCommonJS(getCustomersDropdown_exports);
...
async function GET(request) {
  try {
    ...
    return import_server.NextResponse.json({ success: true, data }, { status: 200 });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to fetch customers";
    console.error("getCustomersDropdown handler error:", message);
    return import_server.NextResponse.json({ success: false, error: message }, { status: 500 });
  }
}
