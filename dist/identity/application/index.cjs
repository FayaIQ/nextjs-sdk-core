...
// src/identity/application/index.ts
var application_exports = {};
__export(application_exports, {
  GETApplicationsStoreDropDown: () => GET3,
  GETStoreInfo: () => GET2,
  GETStores: () => GET,
  getApplicationsStoreDropdown: () => getApplicationsStoreDropdown,
...
async function getStores() {
  if (typeof window === "undefined") {
    ...
    const { Api: Api2 } = await Promise.resolve().then(() => (init_api(), api_exports));
    return getWithAuth2(Api2.getApplicationsStores);
  }
...
async function GET(request) {
  try {
    ...
    return import_server.NextResponse.json(stores);
  } catch (error) {
...
async function GET2() {
  try {
    ...
    return import_server2.NextResponse.json(storeInfo);
  } catch (error) {
