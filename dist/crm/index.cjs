...
__export(crm_exports, {
  GETClients: () => GET,
  GETClientsPaging: () => GET2,
  POSTClient: () => POST,
  getClients: () => getClients,
  getClientsPaging: () => getClientsPaging,
...
async function getClientsPaging(query) {
  ...
  if (typeof window === "undefined") {
    ...
    const { Api: Api2 } = await Promise.resolve().then(() => (init_api(), api_exports));
    return getWithAuth2(`${Api2.getClientsPaging}${qs}`);
  }
...
async function getClients({ filterParams } = {}) {
  ...
  if (typeof window === "undefined") {
    const { getWithAuth: getWithAuth2 } = await Promise.resolve().then(() => (init_fetcher(), fetcher_exports));
    const { Api: Api2 } = await Promise.resolve().then(() => (init_api(), api_exports));
    return getWithAuth2(`${Api2.getClients}${qs}`);
  }
...
async function postClient(data) {
  if (typeof window === "undefined") {
    ...
    const { Api: Api2 } = await Promise.resolve().then(() => (init_api(), api_exports));
    return postWithAuth2(Api2.postClients, data);
  }
...
async function POST(request) {
  try {
    ...
    return import_server4.NextResponse.json(client, { status: 201 });
  } catch (err) {
    return toNextResponseFromError(err);
  }
}
