...
async function apiFetch(url, options = {}) {
  ...
  if (options.cache) fetchInit.cache = options.cache;
  if (options.next && (typeof options.next.revalidate === "number" || options.next.tags && options.next.tags.length)) {
    fetchInit.next = {
...
async function getWithAuth(url, query, headers, nextOptions) {
  let token = null;
  try {
  ...
  return apiFetch(url, {
    ...
    query,
    headers,
    next: nextOptions ? { revalidate: nextOptions.revalidate, tags: nextOptions.tags } : void 0,
    cache: nextOptions?.cache
  });
}
async function getWithoutAuth(url, query, headers, nextOptions) {
  return apiFetch(url, {
    method: "GET",
    query,
    headers,
