...
async function startPhoneSignIn(phoneNumber, options) {
  ...
  const primaryAppForProject = await getPrimaryApp();
  const defaultProjectName = primaryAppForProject?.options?.projectId || "ozoon";
  ...
  return {
    confirm: async (code) => {
      ...
      try {
        ...
        const { getPrimaryApp: getPrimaryApp2 } = await import("./config-QT4SQ6SK.js");
